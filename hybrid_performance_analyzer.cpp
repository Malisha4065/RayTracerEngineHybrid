...
// CPU-only OpenMP performance test function
void run_cpu_openmp_performance_test(int num_threads, FILE* csv_file) {
    printf("\n=== Testing CPU OpenMP Configuration ===\n");
    printf("CPU threads: %d\n", num_threads);
    
    // Allocate result buffers
    Vec3* cpu_serial_result = (Vec3*)malloc(TEST_WIDTH * TEST_HEIGHT * sizeof(Vec3));
    Vec3* cpu_openmp_result = (Vec3*)malloc(TEST_WIDTH * TEST_HEIGHT * sizeof(Vec3));
    ...
    // CPU OpenMP timing
    double cpu_openmp_total_time = 0.0;
    for (int i = 0; i < NUM_ITERATIONS; i++) {
        auto start = std::chrono::high_resolution_clock::now();
        render_cpu_openmp(cpu_openmp_result, TEST_WIDTH, TEST_HEIGHT, test_spheres, 3, cam, base_seed, num_threads);
        auto end = std::chrono::high_resolution_clock::now();
        
        double time_ms = std::chrono::duration<double, std::milli>(end - start).count();
        cpu_openmp_total_time += time_ms;
    }
    double avg_cpu_openmp_time = cpu_openmp_total_time / NUM_ITERATIONS;
    
    // Calculate accuracy metrics
    double mse = calculate_mse(cpu_openmp_result, cpu_serial_result, TEST_WIDTH, TEST_HEIGHT);
    double rmse = calculate_rmse(cpu_openmp_result, cpu_serial_result, TEST_WIDTH, TEST_HEIGHT);
    double psnr = 10.0 * log10(3.0 / mse);
    double speedup = avg_cpu_serial_time / avg_cpu_openmp_time;
    
    // Print results
    printf("Serial Time: %.2f ms\n", avg_cpu_serial_time);
    printf("OpenMP Time (%d threads): %.2f ms\n", num_threads, avg_cpu_openmp_time);
    printf("OpenMP Speedup: %.2fx\n", speedup);
    printf("Accuracy - RMSE: %.6f, PSNR: %.2f dB\n", rmse, psnr);
    
    // Write to CSV
    fprintf(csv_file, "%d,CPU_ONLY,%.2f,%.2f,%.2f,%.6f,%.6f,%.2f\n",
            num_threads, avg_cpu_serial_time, avg_cpu_openmp_time, speedup,
            mse, rmse, psnr);
    
    // Cleanup
    free(cpu_serial_result);
    free(cpu_openmp_result);
    
    printf("----------------------\n");
}

// GPU-only performance test function
void run_gpu_only_performance_test(int gpu_threads_x, int gpu_threads_y, FILE* csv_file) {
    printf("\n=== Testing GPU-Only Configuration ===\n");
    printf("GPU threads: %dx%d\n", gpu_threads_x, gpu_threads_y);
    
    // Calculate GPU configuration details
    int threads_per_block = gpu_threads_x * gpu_threads_y;
    int grid_x = (TEST_WIDTH + gpu_threads_x - 1) / gpu_threads_x;
    int grid_y = (TEST_HEIGHT + gpu_threads_y - 1) / gpu_threads_y;
    int total_blocks = grid_x * grid_y;
    ...
    for (int i = 0; i < NUM_ITERATIONS; i++) {
        auto start = std::chrono::high_resolution_clock::now();
        render_gpu_cuda(gpu_result, TEST_WIDTH, TEST_HEIGHT, test_spheres, 3, cam, base_seed, gpu_threads_x, gpu_threads_y);
        auto end = std::chrono::high_resolution_clock::now();
        
        double time_ms = std::chrono::duration<double, std::milli>(end - start).count();
        gpu_total_time += time_ms;
    }
    double avg_gpu_time = gpu_total_time / NUM_ITERATIONS;
    
    // Calculate accuracy metrics
    double mse = calculate_mse(gpu_result, cpu_serial_result, TEST_WIDTH, TEST_HEIGHT);
    double rmse = calculate_rmse(gpu_result, cpu_serial_result, TEST_WIDTH, TEST_HEIGHT);
    double psnr = 10.0 * log10(3.0 / mse);
    double speedup = avg_cpu_serial_time / avg_gpu_time;
    
    // Print results
    printf("Serial Time: %.2f ms\n", avg_cpu_serial_time);
    printf("GPU Time (%dx%d): %.2f ms\n", gpu_threads_x, gpu_threads_y, avg_gpu_time);
    printf("GPU Speedup: %.2fx\n", speedup);
    printf("Accuracy - RMSE: %.6f, PSNR: %.2f dB\n", rmse, psnr);
    
    // Write to CSV
    fprintf(csv_file, "N/A,%dx%d,%.2f,%.2f,%.2f,%.6f,%.6f,%.2f\n",
            gpu_threads_x, gpu_threads_y, avg_cpu_serial_time, avg_gpu_time, speedup,
            mse, rmse, psnr);
...
int main() {
    ...
    printf("\n=== PHASE 1: CPU OpenMP Performance Testing ===\n");
    printf("Testing CPU OpenMP performance with different thread counts...\n");
    
    // Test CPU OpenMP performance first
    for (int cpu_threads : thread_counts) {
        run_cpu_openmp_performance_test(cpu_threads, csv_file);
    }
    
    printf("\n=== PHASE 2: GPU-Only Performance Testing ===\n");
    printf("Testing GPU performance with different thread configurations...\n");
    
    // Test GPU-only performance
    for (const auto& gpu_config : gpu_configs) {
        ...
        if (valid_config) {
            run_gpu_only_performance_test(gpu_config.first, gpu_config.second, csv_file);
        } else {
            printf("Skipping GPU:%dx%d configuration (not suitable for device: %d threads per block)\n", 
                   gpu_config.first, gpu_config.second, threads_per_block);
        }
    }
    
    printf("\n=== PHASE 3: Hybrid Performance Testing ===\n");
    ...
    printf("- Scene: 3 spheres (same as CUDA reference)\n");
    printf("- Camera: Fixed position (same as CUDA reference)\n");
    printf("- Lighting: Lambertian diffuse (same as CUDA reference)\n");
    printf("- Random seed: 1234 (same as CUDA reference)\n");
    printf("\nThreading Analysis:\n");
    printf("- CPU OpenMP threads tested: %zu different counts\n", thread_counts.size());
    printf("- GPU configurations tested: %zu different thread block sizes\n", gpu_configs.size());
    printf("- Total hybrid combinations: %zu\n", thread_counts.size() * gpu_configs.size());
    printf("\nKey Metrics Explained:\n");
    printf("- Speedup: How much faster method is compared to serial CPU\n");
    ...
    printf("\nGPU Thread Organization (same as CUDA reference):\n");
    printf("- Each GPU thread processes exactly 1 pixel\n");
    printf("- Total GPU threads = (Grid_X × Grid_Y) × (Threads_X × Threads_Y)\n");
    ...
    printf("- High speedup (>10x for good GPU config)\n");
    printf("- Low RMSE (<0.1)\n");
    printf("- High PSNR (>20 dB)\n");
    printf("- GPU time > 1ms (avoid suspiciously low times)\n");
    printf("- Efficient use of both CPU and GPU resources\n");
    printf("\nComparison with CUDA Reference:\n");
    printf("- Same scene, camera, lighting, sampling, and random seed\n");
    printf("- Same GPU thread configurations tested\n");
    printf("- Same accuracy metrics calculated\n");
    printf("- Additional hybrid CPU+GPU combinations tested\n");
    printf("- Results directly comparable to CUDA-only implementation\n");
